...
contract FakeNewsDApp {
    ...
    mapping(address => mapping(string => uint256)) weights;         // weights for votes [every category separate]
    mapping(address => uint) correct_votes;                         // total correct votes
    mapping(address => uint) incorrect_votes;                       // total incorrect votes
    mapping(address => uint256) reputations;                        // reputation score as weightage for rewards re-distribution
    mapping(address => uint) probation;                             // probabtion period ratio of weight

    // Mapping to store news items & if voting period active
    mapping(string => NewsItem) newsItems;
    mapping(string => bool) voting;
    ...
    function uploadNewsItem(string memory _contenturl, string _category) onlyFactChecker {
        // bytes32 itemurl = keccak256(abi.encodePacked(_contenturl, block.timestamp, msg.sender));
        string itemurl = _contenturl;
        require(newsItems[itemurl].uploader == address(0), "Item already uploaded");

        // Category can be implemented on the front-end part
        if (! findCategory(_category)) {
            add_new_category(_category);
        }

        // add a new news item
        NewsItem memory newItem;
        newItem.uploader = msg.sender;
        newItem.contenturl = _contenturl;
        ...
        newsItems[itemurl] = newItem;
        emit NewsItemUploaded(newItem);
    }

    function start_voting(string _itemurl) {
        voting[_itemurl] = true;
    }
    function stop_voting(string _itemurl) {
        voting[_itemurl] = false;
        voting_results(newsItems[_itemurl]);
    }


    // Function for fact-checkers to vote on an item
    function voteOnNewsItem(string _itemurl, uint8 _vote) onlyFactChecker {
        require(newsItems[_itemurl].uploader != address(0), "Item does not exist");
        require(voting[_itemurl], "Voting Period not active");

        // --- REQUIRE A DEPOSIT OF 'x' cryptocurrency

        // store the vote
        newsItems[_itemurl].votes[msg.sender] = _vote;
        newsItems[_itemurl].totalVotes += 1;
        newsItems[_itemurl].voters.push(msg.sender);
    }
    ...
    function getTruthfulnessScore(string _itemurl) public view returns (uint256) {
        require(newsItems[_itemurl].uploader != address(0), "Item does not exist");
        return newsItems[_itemurl].fakenessScore;
    }
    ...
    function getNewsItemCategory(string _itemurl) public view returns (string) {
        return newsItems[_itemurl].category;
    }
