...
class Transaction :
    ...
    def __init__(self, sender, receiver, coins, time):
        self.timestamp = time
...
class Peer :
    ...
    async def generate(self):
        time = 0
        while time < full_simulation_time:
    ...
    async def broadcast(self, msg , time = 0 ):
        ...
        if msg not in queue:
               ...
               for peer in self.connections :
                   ...
                   tasks.append( peer.broadcast(msg , time + latency) )

               if isinstance(msg,Block) :
                   block = msg
                   if self.blockchain.add_block(block) :  # This function returns if it creates a new long chain 
                        tasks.append( self.create_and_publish_block(time) ) # Create a block and add to publish thread 
                   else : 
    ...
    async def create_and_publish_block(self, time = 0) :
        ...
        block = Block( self.blockchain.get_last_block().blkid , time + tk , transactions , self.id )
...
async def main() :
    ...
    await asyncio.gather( *publish_block , *generate_txns )
